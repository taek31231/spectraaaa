...
setup_fig = go.Figure()
setup_fig.add_shape(type="rect", x0=3, y0=0.5, x1=3.2, y1=1.5, fillcolor="black")
setup_fig.add_trace(go.Scatter(x=[5], y=[1], mode="markers", marker=dict(size=60, symbol="triangle-up", color="lightgray"), showlegend=False))
setup_fig.add_shape(type="rect", x0=8, y0=0, x1=8.2, y1=2, fillcolor="gray", opacity=0.3)

if spectrum_type == "연속 스펙트럼":
    setup_fig.add_trace(go.Scatter(x=[1], y=[1], mode="markers+text", marker=dict(size=40, color="orange", symbol="star"), text=["광원(별)"], textposition="bottom center"))
    ...
    setup_fig.add_trace(go.Scatter(x=[0.5], y=[1], mode="markers", marker=dict(size=30, color="orange", symbol="star")))
...
setup_fig.update_layout(xaxis=dict(range=[0, 9], visible=False), yaxis=dict(range=[0, 2], visible=False), height=150, margin=dict(l=0, r=0, t=0, b=0), template="plotly_white")
...
# 서브플롯 생성
fig = make_subplots(rows=2, cols=1, row_heights=[0.4, 0.6], shared_xaxes=True, vertical_spacing=0.1)
