...
setup_fig.add_shape(type="rect", x0=3, y0=0.6, x1=3.1, y1=1.4, fillcolor="black", line_width=0) 
setup_fig.add_trace(go.Scatter(x=[5], y=[1], mode="markers", marker=dict(size=65, symbol="triangle-up", color="lightgray"), showlegend=False)) 
setup_fig.add_shape(type="rect", x0=8, y0=0.2, x1=8.1, y1=1.8, fillcolor="gray", opacity=0.2) 
...
if spectrum_type == "연속 스펙트럼":
    ...
    setup_fig.add_trace(go.Scatter(x=[0.7, 3, 5], y=[1, 1, 1], mode="lines", line=dict(color="orange", width=3)))
    for i, color in enumerate(['violet', 'blue', 'green', 'yellow', 'red']):
        # 에러 수정 지점: opacity를 line 밖으로 뺌
...
with col1:
    st.markdown("### 🌈 연속 스펙트럼")
    st.write("**원리:** 고온의 고체나 별의 광구처럼 밀도가 높은 물체가 방출하는 빛입니다.")
    st.write("**특징:** 모든 파장에서 빛이 나오므로 매끄러운 무지개색 띠가 나타납니다.")

with col2:
    st.markdown("### ✨ 방출 스펙트럼")
    st.write(f"**원리:** 가열된 **고온의 저밀도 기체**가 특정 에너지를 방출할 때 나타납니다.")
    st.write("**특징:** 검은 바탕에 원소 고유의 밝은 선이 나타나며, 그래프에 '산'이 생깁니다.")

with col3:
    st.markdown("### 🌑 흡수 스펙트럼")
    st.write(f"**원리:** 별빛이 **저온의 기체**를 통과할 때 특정 파장이 흡수되며 나타납니다.")
    st.write("**특징:** 무지개 배경에 검은 선이 나타나며, 그래프에 '골'이 생깁니다.")

st.success("📝 **결론:** 스펙트럼의 선 위치를 분석하면 멀리 있는 별의 구성 성분을 알 수 있습니다.")
